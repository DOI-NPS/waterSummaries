...
try(importData(type = "zip", filepath = "../data/records-2313941.zip"))
try(importData(type = "zip", filepath = "./data/records-2313941.zip"))
```
...
```{r }
# park = "ACAD"; yr_start = 2006; yr_current = 2025;
parkcode <- "ACAD" #child_env$park
park = parkcode
# try(yr_start <- params$year_start) # turn on if running rms
# try(yr_current <- params$year_current) # turn on if running rmd
# yr_start = 2006
# yr_current = 2024
yr_prev <- yr_current - 1
yr_4yr <- yr_current - 2
site_df1 <- unique(getEvents(park = "ACAD", years = yr_4yr:yr_current)[, c("SiteCode", "SiteName", "UnitCode", "UnitName", "SiteType", "year")])
# take most recent site/year combo for core sites
site_df <- site_df1 |> slice_max(order_by = year, by = c(SiteCode, SiteName, UnitCode, UnitName, SiteType))
...
source("fake_bands_legend_code.R") # for legends called legg and leg_quant
```
...
for(i in seq_along(lake_list)){
  ...
  yr_current <- site_df$year[site_df$SiteCode==lakecode]
  yr_prev <- yr_current - 1
...
for(i in seq_along(param_list)){
  ...
  yr_current = max(site_df$year, na.rm = T)
...
for(i in seq_along(lake_list)){
  ...
  yr_current <- site_df$year[site_df$SiteCode==sitecode]
...
#for(i in seq_along(stream_list[10:17])){
  ...
  yr_current <- site_df$year[site_df$SiteCode==streamcode]
  yr_prev <- yr_current - 1
