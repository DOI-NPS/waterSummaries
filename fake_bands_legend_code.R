...
fake_points_quant <-
  ggplot(point_data[1,], aes(x = month, y = val, color = var))+
  geom_point()+
  labs(y = NULL, x = NULL, title = NULL) + 
  scale_color_manual(values = 
                       c("Current value" = 'black'),
...
leg_points_quant_pb <- as_ggplot(get_legend(fake_points_quant))
...
leg_quant <- as_ggplot(
  grid.arrange(grobs = list(leg_bands_pb, leg_points_quant_pb, leg_lines_pb),
               layout_matrix = rbind(c(1, 1),
                                     c(2, 3))
  ))
leg_quant
